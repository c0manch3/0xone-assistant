...
`dispatch_reply` держит in-memory `_DedupLedger` с TTL=300 s, который
дедуплицирует доставку по ключу `(chat_id, resolved_outbox_path)`. Если
тот же путь появится в тексте дважды в пределах 300-секундного окна
(например, main turn упомянул, а потом follow-up scheduler-trigger
пересказал артефакт), реально улетит только первая `send_photo`;
остальные совпадения отброшены. Ledger — in-memory, **не переживает
рестарт Daemon'а**. Это вторая линия обороны — prompt rule выше
(не упоминать path после `task spawn`) остаётся первой.
