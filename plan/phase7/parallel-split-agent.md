...
## Metadata
- Plan version: detailed-plan.md r2 + implementation.md v2   # (v2 H-15 required marker)
...
## Wave 6 — TelegramAdapter + handler/bridge envelope (v2 fix-pack C-5: PARALLEL, 2 agents)
### Wave 6A — Commit 12: TelegramAdapter media handlers + send_photo/document/audio + attachment dedup (I-7.6)
- Branch: `wt/phase7-commit-12`
- Touches: `src/assistant/adapters/telegram.py` (+200 LOC only)
- Depends: commits 4 + 6 merged.
- Agent prompt: adapter media handlers per implementation.md §3.4. Attach adapter-level dedup per I-7.6 (60 s TTL, 128 LRU). RetryAfter + FileNotFoundError handling per L-20/L-21.
### Wave 6B — Commit 13: Handler + bridge multimodal envelope
- Branch: `wt/phase7-commit-13`
- Touches: `src/assistant/handlers/message.py`, `src/assistant/bridge/claude.py`, `src/assistant/bridge/history.py` (+105 LOC, disjoint from 6A)
- Depends: commits 4 + 11 merged.
- Agent prompt: envelope builder per implementation.md §3.1, §3.2, §3.3. Include explicit `photo_mode=="path_tool"` branch (C-4) + turn-id placeholder (H-10).

Waves 6A and 6B run in parallel — file sets are disjoint (adapter vs handler/bridge). Both depend on commit 4 (shared types in `adapters/base.py`) but that is already merged by this point.
...
**(v2 fix-pack, H-15) Pre-condition — wave-plan.md generation:**
parallel-split agent runs ONLY AFTER researcher fix-pack v2 commits
`implementation.md` final. Orchestrator MUST NOT invoke parallel-split
before fix-pack merge. Any `wave-plan.md` generated from v1 (pre-fix-pack)
is STALE and MUST be invalidated:

- Orchestrator detects v1 wave-plan by header `Plan version:
  detailed-plan.md r1 + implementation.md v1` (or absent v2 marker).
- On v2 fix-pack merge, orchestrator deletes any existing
  `plan/phase7/wave-plan.md` and re-invokes parallel-split.
- Fix-pack v2 changes that invalidate v1 wave-plan:
  1. New commit 2b inserted (root `pyproject.toml` dep addition, C-1).
  2. Wave 6 split into parallel Wave 6A + 6B (C-5).
  3. Commit 15 factory signature simplified (H-11) — affects hook-switch
     agent prompt.
  4. Expanded test matrix for commit 13 (C-4, H-10, H-14, C-2).

A stale v1 wave-plan would:
- Skip commit 2b entirely → coder Wave A (tool commits) fails with
  `ModuleNotFoundError: No module named 'pypdf'` on first test.
- Waste the Wave 6 parallelism opportunity.
- Misdirect commit-15 coder to thread an unused `outbox_root` arg.
...
4. Spike 0, _memlib refactor, AND commit 2b (root pyproject.toml dep addition — v2 fix-pack C-1) — ALWAYS single-agent sequential (wave 1, wave 2, wave 2b).
...
9. **v2 fix-pack C-5:** commits 12 and 13 run in PARALLEL as Wave 6A (adapters/telegram.py) and Wave 6B (handlers/, bridge/) — file sets disjoint. Do NOT combine them.
10. **v2 fix-pack H-11:** commit 15 factory signature is `make_subagent_hooks(settings, store, picker, adapter, dedup_ledger)` — does NOT include `outbox_root`. Agent prompt for commit 15 must reflect this.
11. **v2 fix-pack H-15:** version header at top of wave-plan.md MUST read `Plan version: detailed-plan.md r2 + implementation.md v2` so orchestrator can detect staleness.
