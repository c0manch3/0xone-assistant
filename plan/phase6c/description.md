...
| H6 | Empty caption + voice ≤2 min intent unclear | **Hotfix-2 2026-04-28**: H6 prefix REMOVED. Live smoke (claude-opus-4-7) интерпретировал meta-фразу `"[голосовое от owner — ...]"` как «аудио отсутствует, только маркер от обвязки» → отказывался отвечать на сам вопрос в транскрипции. Теперь: empty caption + voice ≤2 min → `user_text = transcript` verbatim. Модель видит транскрипт как обычный owner-message и отвечает по содержанию. (Original H6 hypothesis: prefix даёт hint how to respond — на практике backfir'ил.) |
